...
def information():
    counter_hybrid_measures = ['counter hybrid by conventional threat',
                           'counter hybrid by cyber attack','counter hybrid by new legislation', 'counter hybrid international collaboration']
    ...
    st.markdown("""2) Deploy cyber weapons within the adversary's critical infrastructure.""")
    st.markdown("""3) On the policy side, state A enacted new legislation against fake news, introducing
                    a quick-response judicial review of suspected manipulative content on social media, as
                    well requiring social media platforms to disclose the source of fundings for advertisement through their channels.""")
    st.markdown("""4) At the international level,  seek to foster robust cooperation mechanisms to
                    counter online disinformation and established bodies specifically aimed at tackling the
                    issue. In addition, being a member of a supranational organisation, it advocated for
                    the establishment of a common legislative framework that would require social media
                    platforms to disclose information regarding its algorithms and submit to third-party
                    audit.
                                """)
    ...
    the costs also include political capital for international support, costs for balancing freedom of speech with
    government control, or costs for potential of escalation. The only costs that are NOT included
    are costs that come with effectiveness of the counter hybrid measure. Please rate the counter hybrid measure costs on a scale from 0-100.
               """)
    gamma_1 = st.slider("What are my costs for {}?".format(counter_hybrid_measures[0]),0,100, value= 50, step=10)
    gamma_2 = st.slider("What are my costs for {}?".format(counter_hybrid_measures[1]),0,100, value= 10, step=10)
    gamma_3 = st.slider("What are my costs for {}?".format(counter_hybrid_measures[2]),0,100, value= 15, step=10)
    gamma_4 = st.slider("What are my costs for {}?".format(counter_hybrid_measures[3]),0,100, value= 10, step=10)
    ...
    st.write("""Hybrid conducts entail many uncertain elements. A failure of detection of the hybrid conduct can result
     in disastrous effects by letting the adversary commit in meddling in the elections in a way that favors the adversary
     without being aware. Similarly, a failure in attribution can result in a lack of success in gaining enough political 
     support to take effective counter-measures. Because the nature of a hybrid conduct is to remain under the threshold 
     of detection and attribution and one can never be 100% certain that we detect or attribute correctly, the possible
     outcomes should be modeled probabilistically.
    """)
    ...
    st.write("""_Reasoning default: the idea was to deter the adversary for conducting a hybrid attack, but since that
    did not work, a conventional attack should follow to not lose credibility. Either way, the consequences of the conventional attack 
    are unpredictable._
    """)

    w111 = st.slider("What is the likelihood of pay-off loss {} when deterrer commits to {} and attacker conducts {}?"
                     .format(theta_1, counter_hybrid_measures[0],attack_measures[0] ), 0,100, 30, step=10)
    w112 = st.slider("What is the likelihood of pay-off loss {} when deterrer commits to {} and attacker conducts {}?"
                     .format(theta_2, counter_hybrid_measures[0],attack_measures[0] ), 0,100, 30,  step=10)
    w113 = st.slider("What is the likelihood of pay-off loss {} when deterrer commits to {} and attacker conducts {}?"
                     .format(theta_3, counter_hybrid_measures[0],attack_measures[0] ), 0,100, 40, step=10)
    ...
    w211 = st.slider("What is the likelihood of pay-off loss {} when deterrer commits to {} and attacker conducts {}?"
                     .format(theta_1, counter_hybrid_measures[1],attack_measures[0] ), 0,100, 80, step=10)
    ...
    w213 = st.slider("What is the likelihood of pay-off loss {} when deterrer commits to {} and attacker conducts {}?"
                     .format(theta_3, counter_hybrid_measures[1],attack_measures[0] ), 0,100, 0, step=10)
    ...
    w311 = st.slider("What is the likelihood of pay-off loss {} when deterrer commits to {} and attacker conducts {}?"
                     .format(theta_1, counter_hybrid_measures[2],attack_measures[0] ), 0,100, 20, step=10)
    w312 = st.slider("What is the likelihood of pay-off loss {} when deterrer commits to {} and attacker conducts {}?"
                     .format(theta_2, counter_hybrid_measures[2],attack_measures[0] ), 0,100, 30, step=10)
    w313 = st.slider("What is the likelihood of pay-off loss {} when deterrer commits to {} and attacker conducts {}?"
                     .format(theta_3, counter_hybrid_measures[2],attack_measures[0] ), 0,100, 50, step=10)
    ...
    st.write("""_Reasoning default: finding international partners that pressure social media organisations to disclose 
    information about algorithms and bots increases the probability of detection and therefore reduces the worst possible
    outcomes._
    """)

    w411 = st.slider("What is the likelihood of pay-off loss {} when deterrer commits to {} and attacker conducts {}?"
                     .format(theta_1, counter_hybrid_measures[3],attack_measures[0] ), 0,100, 10, step=10)
    w412 = st.slider("What is the likelihood of pay-off loss {} when deterrer commits to {} and attacker conducts {}?"
                     .format(theta_2, counter_hybrid_measures[3],attack_measures[0] ), 0,100, 60, step=10)
    ...
    a_1 = st.slider("How effective is {}".format(counter_hybrid_measures[0]), 0,100, 90, step = 10)*0.01
    a_2 = st.slider("How effective is {}".format(counter_hybrid_measures[1]), 0,100, 40, step = 10)*0.01
